...
class BorrowerRepoTest {

    private final static String USER = "postgres";
    private final static String PASSWORD = "postgres";
    private final static String URL = "jdbc:postgresql://localhost:5432/libary?currentSchema=test";

    private static DatabaseConnection connector;
    private static BorrowerRepo borrowerRepo;

    @BeforeAll
    public static void setUpClass() {
        try {
            connector = new DatabaseConnection(USER, PASSWORD, URL);
            // dependency injection
            borrowerRepo = new BorrowerRepo(connector);
    ...
    @Test
    void findBorrowerById() throws DatabaseException{

        Borrower expected = new Borrower(1,"Anders Sørensen", "Nørregade 10", 8000);
        assertEquals(expected, borrowerRepo.findBorrowerById(1));

//        String name = "Anders Sørensen";
//        String address = "Nørregade 10, 8000";
//        int id = 1;
//
//        assertEquals(name, borrowerRepo.findBorrowerById(1).getName());
//        assertEquals(address, borrowerRepo.findBorrowerById(1).getAddress());
//        assertEquals(1, borrowerRepo.findBorrowerById(1).getBorrower_id());
    }

    @Test
    void getAllBorrowerWithAddress() {
    }

    @Test
    void insertNewBorrower() throws DatabaseException {
        Borrower borrower = new Borrower("Alissa Storm", "Voltvej 5", 8000);
        borrowerRepo.InsertNewBorrower(borrower);
        String name = "Alissa Storm";
        String address = "Voltvej 5";

        assertEquals(name, borrower.getName());
        assertEquals(address, borrower.getAddress());
        //get all loaners, see if it got bigger after the new insert
        //insert and find the loaner with the variables to see if its correct
    }
}